...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
	var isInvokedFromLocalNotification:Bool = false;

    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
		if let opts = launchOptions{
			if opts[UIApplicationLaunchOptionsLocalNotificationKey] != nil{
				isInvokedFromLocalNotification = true;
			}
		}
		else
		{
			isInvokedFromLocalNotification = false;
		}
